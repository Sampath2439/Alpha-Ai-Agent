...
export const handleProgressStream: RequestHandler = (req, res) => {
  ...
  const handleJobCompleted = (progress: any) => {
    res.write(`data: ${JSON.stringify({ type: 'completed', data: progress })}\n\n`);
  };

  const handleJobFailed = (progress: any) => {
    res.write(`data: ${JSON.stringify({ type: 'failed', data: progress })}\n\n`);
  };

  const handleJobQueued = (progress: any) => {
    res.write(`data: ${JSON.stringify({ type: 'queued', data: progress })}\n\n`);
  };
