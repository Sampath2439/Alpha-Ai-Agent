    ...
    'TechCorp Solutions': [
      {
        ...
        snippet: 'TechCorp Solutions provides enterprise-grade cloud infrastructure and DevOps automation tools. Our platform helps companies scale their applications with 99.9% uptime guarantee.'
      },
      {
        url: 'https://techcorp.com/pricing',
        title: 'TechCorp Pricing - Flexible Plans',
        snippet: 'Choose from our Starter ($99/month), Professional ($299/month), or Enterprise (custom pricing) plans. All plans include 24/7 support and advanced monitoring.'
      },
      {
        url: 'https://techcrunch.com/techcorp-funding',
        title: 'TechCorp Raises $50M Series B',
        snippet: 'TechCorp competes with AWS, Google Cloud, and Azure in the cloud infrastructure space. Key differentiators include simplified DevOps workflows and cost optimization.'
      }
    ],
    'TechCorp products': [
      {
        ...
        snippet: 'Our flagship products include CloudDeploy for automated deployments, MonitorPro for application monitoring, and ScaleMaster for auto-scaling infrastructure.'
      },
      {
        url: 'https://docs.techcorp.com',
        title: 'TechCorp Documentation',
        snippet: 'Complete documentation for CloudDeploy, MonitorPro, and ScaleMaster products with API references and integration guides.'
      }
    ],
    'TechCorp competitors': [
      {
        ...
        snippet: 'Major competitors in the cloud infrastructure space include AWS, Google Cloud Platform, Microsoft Azure, DigitalOcean, and Heroku.'
      },
      {
        url: 'https://techcorp.com/vs-competition',
        title: 'TechCorp vs Competition',
        snippet: 'TechCorp differentiates from AWS and Azure through simplified deployment processes and 40% lower costs for mid-market companies.'
      }
    ]
  };
...
export class ResearchAgent {
  ...
  async enrichPerson(personId: string): Promise<ResearchPayload> {
    ...
    for (let iteration = 1; iteration <= this.maxIterations; iteration++) {
      ...
      if (missingFields.length === 0) {
        console.log(`Research completed for ${person.full_name} after ${iteration - 1} iterations`);
        break;
      }

      const query = this.generateQuery(company, person, missingFields, iteration);
  ...
  private getMissingFields(payload: ResearchPayload): string[] {
    ...
    return requiredFields.filter(field => !payload[field as keyof ResearchPayload]);
  }

  private generateQuery(company: Company, person: Person, missingFields: string[], iteration: number): string {
  ...
  private extractInformation(searchResults: SearchResult[], payload: ResearchPayload): void {
    const allText = searchResults.map(r => `${r.title} ${r.snippet}`).join(' ').toLowerCase();

    // Extract value proposition
    if (!payload.company_value_prop) {
      if (allText.includes('platform') || allText.includes('solution') || allText.includes('helps')) {
        const snippet = searchResults.find(r => 
    ...
    if (!payload.product_names) {
      ...
      const productKeywords = ['clouddeploy', 'monitorpro', 'scalemaster', 'platform', 'product'];
    ...
    if (!payload.pricing_model) {
      if (allText.includes('$') || allText.includes('pricing') || allText.includes('plan')) {
        const pricingSnippet = searchResults.find(r => 
          r.snippet.includes('$') || 
    ...
    if (!payload.key_competitors) {
      ...
      const competitorKeywords = ['aws', 'google cloud', 'azure', 'digitalocean', 'heroku', 'competitor'];
      
      for (const result of searchResults) {
        const text = result.snippet.toLowerCase();
        competitorKeywords.forEach(keyword => {
          if (text.includes(keyword) && keyword !== 'competitor') {
            competitors.push(keyword.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '));
          }
        });
      }
